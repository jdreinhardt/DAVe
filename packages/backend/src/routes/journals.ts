...
export async function journalsRoutes(
  ...
  app.get<{ Params: { uid: string } }>(
    ...
    async (req, reply) => {
      ...
      const journal = rowToVJournalEntry(
        row,
  ...
  app.post<{ Body: CreateNoteRequest }>(
    ...
    async (req, reply) => {
      ...
      try {
        ...
        result = await davCreateJournal(session, target, journalData, config);
      } catch (err: unknown) {
        ...
        if (isMove) {
          ...
          try {
            ...
            app.log.error(
              ...
              'Move copied the journal but could not remove the original; it now exists in both collections',
            );
          }
        } else {
